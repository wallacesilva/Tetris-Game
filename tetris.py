...
def play():
    ...
    while run:
        ...
        if check_lost(locked_positions):
            run = False
    window.fill((27, 31, 29 ))
    draw_text_middle("GAME OVER !", 70, (255,255,255), window)
    pygame.display.update()
...
def game():
    ...
    while run:
        window.fill (( 0,0,0 ))
        window.blit(bimg, (0,0))
        draw_text_middle("Press any key to play!", 70, (0, 0, 0), window)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
...
bimg = pygame.image.load("start.jpg")
bimg = pygame.transform.scale(bimg, (w_width, w_height)).convert_alpha()

bacimg = pygame.image.load("tetris.jpg")
bacimg = pygame.transform.scale(bacimg, (w_width, w_height)).convert_alpha()
